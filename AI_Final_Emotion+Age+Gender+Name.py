...
while True:
    ...
    for (x,y,h,w) in faces_detected:
        ...
        cv2.putText(frame, emotion_prediction, (x, y), fontFace=cv2.FONT_HERSHEY_SIMPLEX, 
                    fontScale=1, color=(0,255,255), thickness=1)
        # Label Name
        n_predict = model2.predict(roi_gray)
        cv2.rectangle(frame, (x, y), (x + w, y + h), (0, 255, 0), 3)
        if n_predict[1]<150:
            cv2.putText(frame, '% s' %(names[n_predict[0]]), (x, y-20),cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), thickness = 1);
        else:
            cv2.putText(frame, 'not recognized',(x, y-20), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0))
        cv2.putText(frame, overlay_text, (x, y-40), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2, cv2.LINE_AA)
